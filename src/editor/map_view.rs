...
pub fn check_map_unit_load(
    asset_server: Res<AssetServer>,
    query: Query<(Entity, &MapUnit), With<LoadingMapUnit>>,
    ...
    for (entity, map_view) in query.iter() {
        ...
        entity.insert(LoadingChipSet(texture));
    }
}

pub fn check_image_load(
    asset_server: Res<AssetServer>,
    ...
    for (entity, map_unit, chipset) in query.iter() {
        let handle = match asset_server.get_load_state(&chipset.0) {
            ...
            Some(LoadState::Failed(_)) => Some(null_chipset.0.clone()),
            Some(LoadState::NotLoaded) | None => {
