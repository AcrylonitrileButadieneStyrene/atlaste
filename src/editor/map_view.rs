...
pub fn on_add(
    ...
    let map = asset_server.load::<MapUnitAsset>(game_path.0.join(&file));
    match asset_server.load_state(&map) {
        LoadState::NotLoaded | LoadState::Loading => {
            commands.spawn((MapUnit(map), Loading));
        }
        LoadState::Loaded => {
            let entity = commands.spawn(MapUnit(map)).id();
...
pub fn process_loading(
    ...
    query.iter().for_each(|(entity_id, map)| {
        if matches!(asset_server.load_state(&map.0), LoadState::Loaded) {
            if let Some(mut entity) = commands.get_entity(entity_id) {
                entity.remove::<Loading>();
                entity.trigger(Loaded(entity_id));
            }
        }
    });
}
...
pub fn on_loaded(
    ...
    let map_unit = map_unit.get(trigger.0).unwrap();
    ...
    let texture = asset_server.load::<Image>(game_path.0.join("ChipSet").join(file + ".png")); // TODO: it can be a .bmp too
    ...
    for (x, y) in itertools::iproduct!(0..size.x, 0..size.y) {
        ...
        let tile_entity = commands
            .spawn(TileBundle {
                position: tile_pos,
                tilemap_id: TilemapId(trigger.0),
    ...
    commands.entity(trigger.0).insert(TilemapBundle {
        tile_size: TILE_SIZE,
        grid_size: GRID_SIZE,
